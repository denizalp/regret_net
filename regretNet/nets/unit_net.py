...
class Net(BaseNet):
    ...
    def build_net(self):
        ...
        with tf.variable_scope("alloc"):
            ...
            self.wi_a = create_var(wname, [num_a_hidden_units, (num_agents + 1) * (num_items + 1)], initializer = w_init,  wd = wd)
            ...
            self.bi_a = create_var(wname, [(num_agents + 1) * (num_items + 1)], initializer = b_init)
    ...
    def inference(self, x):
        ...
        agent = tf.nn.softmax(tf.reshape(agent, [-1, self.config.num_agents + 1, self.config.num_items + 1]), axis = 1)

        item = tf.matmul(a, self.wi_a) + self.bi_a
        item = tf.nn.softmax(tf.reshape(item, [-1, self.config.num_agents + 1, self.config.num_items + 1]), axis = -1)

        a = tf.minimum(agent, item)
